#!/usr/bin/env bash
#
# create-site.sh — генератор сайта «Вина Болгарии».
#
# Создаёт всю структуру проекта с нуля: папки assets/ и цельный index.html.
# Запускать ничего устанавливать не нужно — только bash (есть в macOS и Linux).
#
# Использование:
#   ./create-site.sh            # создать сайт в текущей папке
#   ./create-site.sh путь/папка # создать сайт в указанной папке
#
set -euo pipefail

# Папка назначения (по умолчанию — текущая).
DEST="${1:-.}"
mkdir -p "$DEST/assets/photos" "$DEST/assets/logo"

# Плейсхолдеры, чтобы папки попали в git и было понятно, что куда класть.
echo "Кладите сюда фотографии: бутылки, виноградники, блюда." \
  > "$DEST/assets/photos/КЛАДИТЕ_ФОТО_СЮДА.txt"
echo "Кладите сюда логотип представительства." \
  > "$DEST/assets/logo/ЛОГОТИП_СЮДА.txt"

# Сам сайт. Кавычки вокруг 'HTML' отключают подстановку переменных,
# поэтому содержимое записывается дословно.
...
echo "Готово! Сайт создан в папке: $DEST"
echo "Откройте файл $DEST/index.html в браузере."
